            ...
            <div class="row align-items-center">
              <div class="col-md-12">
                <ul class="breadcrumb">
                  ...
                  <li class="breadcrumb-item" aria-current="page">Apps Management</li>
                </ul>
              </div>
              <div class="col-md-12">
                <div class="page-header-title">
                  <h2 class="mb-0">Apps Management</h2>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="col-sm-12">
            ...
            <div class="card-header">
                <div class="d-flex justify-content-between mt-3">
                    <h5 class="w-50">Master Aplikasi</h5>
                    <div class="d-flex justify-content-end w-50">
                        <a href="{{ route('apps') }}" id="add-new" class="btn btn-primary rounded-2">Add New</a>
                    </div>
                </div>
            </div>
              <div class="card-body">
                        ...
                        <tbody id="table-body">
                          @foreach ($apps as $app)
                              <tr>
                                  ...
                                  <td>{{ $app->status }}</td>
                                  <td>
                                    <a href="{{ route('change.update', $app->id) }}" class="btn btn-primary">Update</a>
                                    <a href="{{ route('change.destroy', $app->id) }}" class="btn btn-danger" onclick="event.preventDefault(); document.getElementById('delete-form-{{ $app->id }}').submit();">Delete</a>
                                    <form id="delete-form-{{ $app->id }}" action="{{ route('change.destroy', $app->id) }}" method="POST" style="display: none;">
                                        @csrf
                                        @method('DELETE')
                                    </form>
  ...
  <div class="pct-body" style="height: calc(100% - 85px)">
    ...
    <script>
      ...
      $('#colum-render').DataTable({
        ...
        // Contoh data yang akan ditampilkan ketika klik "Add New"
        const newData = [
        {
            nameApps: 'Finance Tracker',
            picApps: 'John Doe',
            status: 'Active' // Status ditambahkan di sini
        },
        {
            nameApps: 'Project Management',
            picApps: 'Jane Smith',
            status: 'Inactive' // Status ditambahkan di sini
        }
        ];
        ...
        row.innerHTML = `
            ...
            <td>${item.nameApps}</td>
            <td>${item.picApps}</td>
            <td>${item.status}</td> <!-- Tampilkan status di kolom ini -->
            <td>
